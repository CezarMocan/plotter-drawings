...
const drawOutline = ({ x1, y1, r1, x2, y2, r2, direction, radius }, { drawLeft, drawRight, drawCenter }) => {
  ...
  let lengthTotalSplits = parseInt(map(r1, 0.5, 0.05, 10, 2))
  let widthTotalSplits = parseInt(map(r1, 0.5, 0.05, 10, 2))
  /*
  // Shading
  ...
  */
  ...
  return { p, leftStart, leftStop, rightStart, rightStop }
}

const generatePath = (x, y, radius, direction, weight, depth, maxDepth, inBoundsFn, prevEnd = {}) => {

  const splitAngle = Math.PI * (0.15 + random.noise2D(x, y, 1, 0.15))

  while (direction < 0) direction += 2 * Math.PI
  while (direction > 2 * Math.PI) direction -= 2 * Math.PI
  // if (direction >= Math.PI / 4 && direction <= Math.PI - Math.PI / 4) return
  ...
  const endPoint = { 
    x: x + Math.cos(direction) * radius / 20,
    y: y + Math.sin(direction) * radius / 20
  }
  ...
  for (let splitSgn of sgns) {
    let isSplitting = (Math.random() < (0.03 * Math.abs(random.noise3D(x, y, depth / 4, 10, 1))))  
    splitsForSign.push(isSplitting)
  }
  let mainBranchContinues = inBoundsFn(endPoint.x, endPoint.y, radius) && (depth < 2000)
  ...
  let newDirection = direction + random.noise2D(x, y, 5, Math.PI / 20)
  // newDirection += newDirection % (Math.PI / 16)
  ...
  if (mainBranchContinues) {
    const rF = random.noise2D(x, y, 5, 0.06) + 1
    splits.push({ x: newCenter.x, y: newCenter.y, radius: newRadius * rF, direction: newDirection, boundX: endPoint.x, boundY: endPoint.y, sgn: 0 })
  }
    ...
    for (let splitSgn of sgns) {
      ...
      if (!isSplitting) {
        ...
        if (inBoundsFn(newEndPoint.x, newEndPoint.y, newRadius) 
            ...
            && depth + 1 <= maxDepth)
          splits.push({ x: newCenter2.x, y: newCenter2.y, direction: newDirection2, radius: newRadius * 0.55, boundX: newEndPoint.x, boundY: newEndPoint.y, sgn: splitSgn })
  ...
  if (prevEnd.left) {
    const p = createPath()
    p.moveTo(prevEnd.left.x, prevEnd.left.y)
    // if (!contains(allPolys, new paper.Point(p1.leftStart.x, p1.leftStart.y)))
      p.lineTo(p1.leftStart.x, p1.leftStart.y)
    paths.push(p)
  }

  if (prevEnd.right) {
    const p = createPath()
    p.moveTo(prevEnd.right.x, prevEnd.right.y)
    // if (!contains(allPolys, new paper.Point(p1.rightStart.x, p1.rightStart.y)))
      p.lineTo(p1.rightStart.x, p1.rightStart.y)
    paths.push(p)
  }
  ...
  const endSpot = { left: p2.leftStop, right: p2.rightStop }
  // paths.push(parc)

  splits.forEach(({ x, y, radius, direction, boundX, boundY, sgn }) => {    
    generatePath(x, y, radius, direction, weight / splits.length, depth + 1, maxDepth, inBoundsFn, endSpot)
  })
}
...
const sketch = (props) => {
  ...
  for (let index = 0; index < 20; index++) {
      ...
      // generatePath(x, y, 0.5, -Math.PI / 2, 2, 0, 50, inBoundsFn)
      // generatePath(x + 0.5, y, 0.5, -Math.PI / 2, 2, 0, 120, inBoundsFn)
      const direction = Math.PI + random.noise1D(index, 10, Math.PI)
      const rx = Math.random() * width
      const ry = Math.random() * height
      const radius = Math.random() * 0.2 + 0.2
      generatePath(rx, ry, radius, direction, 2, 0, 100, inBoundsFn)
      xxx += 0.5
