...
const generatePath = (x, y, radius, direction, weight, depth, maxDepth, inBoundsFn) => {
  ...
  const splitAngle = Math.PI * (0.35 + random.noise2D(x, y, 1, 0.1))
  ...
  if (mainBranchContinues) {
    splits.push({ x: newCenter.x, y: newCenter.y, radius: newRadius - 0.01, direction: newDirection, boundX: endPoint.x, boundY: endPoint.y, sgn: 0 })
  }
    ...
    for (let splitSgn of sgns) {
      ...
      if (!isSplitting) {
        ...
        if (inBoundsFn(newEndPoint.x, newEndPoint.y, newRadius) 
            ...
            && depth + 1 <= maxDepth
            && !intersectsCircles({ x: newCenter2.x, y: newCenter2.y, radius: newRadius * 0.9 }, circles))
          splits.push({ x: newCenter2.x, y: newCenter2.y, direction: newDirection2, radius: newRadius * 0.9, boundX: newEndPoint.x, boundY: newEndPoint.y, sgn: splitSgn })
...
const sketch = (props) => {
  ...
  for (let i = 0; i < noX; i++) {
    for (let j = 0; j < noY; j++) {
      ...
      generatePath(x, y, 0.35 / 2.8, -Math.PI / 2, 2, 0, 62, inBoundsFn)
    }
  }  
