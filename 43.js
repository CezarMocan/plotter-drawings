...
const sketch = (props) => {
  ...
  const lineWidthWeights = [0.6, 0.1, 0.1]
  const lineWidths = [1, 2, 4]

  const circleWidthWeights = [0.7, 0.3]
  const circleWidths = [1, 15]
  ...
  for (let i = 0; i < N; i++) {
    for (let j = 0; j < M; j++) {
      ...
      // Draw circles
      const noCircles = Math.max(0, Math.round(random.noise2D(i, j, 10, 3) - 0.75))
      ...
      // Draw lines
      const noLines = Math.max(0, Math.round(random.noise2D(i, j, 1, 3) + (3.2 - 2 * noCircles)))
      for (let l = 0; l < noLines; l++) {
        let l = getLineInSquare(
          interpolatePoint({ x: centerX, y: centerY }, p, interpX, interpY), 
          interpolatePoint({ x: centerX, y: centerY }, p2, interpX, interpY), 
          2 + (!!(Math.random() < 0.2)), 2 + !!(Math.random() < 0.2)
        )
