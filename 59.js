...
const createPolygon = (center, radius, noPoints, scale, angleOffset) => {
  ...
  for (let i = 0; i <= noPoints; i++) {
    ...
    polygon.add(new paper.Point(
      Math.cos(angle + angleOffset) * radius * scale.x + center.x + random.noise1D(angle, 0.1, 0.25),
...
const sketch = (props) => {
  ...
  const angleOffset = Math.PI / 4 - 0.2

  const polygon = createPolygon(center, radius, noPoints, scale, angleOffset)

  const centerOffset = { x: 3.0, y: 3.0 }
  // const centerOffset = { x: 0, y: 0.0 }
  ...
  const subdivisionsTotal = 150
  let step = 1 / subdivisionsTotal
  for (let i = 0; i < 1; i += step) {
    const currPoly = new paper.Path()

    centerOffset.x -= Math.sin(i * 80) * 0.235
    centerOffset.y -= Math.cos(i * 130) * 0.07
    const centerSmall = { x: centerOffset.x + center.x, y: centerOffset.y + center.y }
    const centerPolygon = createPolygon(centerSmall, 2.5, noPointsSmall, { x: 0.275, y: 1 }, angleOffset + 4 * (1 - i))

    currPoly.interpolate(polygon, centerPolygon, i)    

    // if (random.noise1D(i, 100, 1) > -0.2)
      // drawPolygon(currPoly)
    
    step += Math.sin(i * 50) * 0.005
  ...
  for (let i = 0; i < allPolys.length - 1; i++) {    
    for (let steps = 0; steps < 1; steps += 0.0075) {
      const noSegments = allPolys[i].segments.length
      const path = createPath()
      for (let j = 0; j < noSegments; j++) {
        // if (random.noise1D(j, 10, 1) > -0.9) continue
        let currPoint = interpolatePoint(allPolys[i].segments[j].point, allPolys[i].segments[(j + 1) % noSegments].point, steps)
        let nextPoint = interpolatePoint(allPolys[i + 1].segments[j].point, allPolys[i + 1].segments[(j + 1) % noSegments].point, steps)
  
        if (random.noise2D(nextPoint.x, nextPoint.y, 10, 1) > -1) {
