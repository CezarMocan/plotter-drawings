...
const splitDraw = (polygon, depth, maxDepth, maxAngle) => {
  ...
  if (Math.abs(polygon.area) < 45) returns = true
  if (Math.random() < 0.3 && depth > 1) returns = true
  ...
  let pctA = 1, pctB = 1
  while (Math.abs(pctA + pctB - 1) > 0.15) {
    pctA = Math.random()
    pctB = Math.random()
  }
...
const sketch = (props) => {
  ...
  for (let i = 0; i < cols; i++) {
    const p = createPath()
    const startPoint = {
      x: map(i, 0, cols, -width * 0.5, width * 1.5),
      y: 0
    }
    const endPoint = {
      x: startPoint.x + 4,
    ...
    for (let j = 0; j <= interp; j++) {
      const step = {
        x: interpolate(startPoint.x, endPoint.x, j / interp),
        y: interpolate(startPoint.y, endPoint.y, j / interp),
      }
      step.x += random.noise2D(step.x, step.y, 0.5, 0.05)
      if (random.noise2D(step.x, step.y, 0.1, 1) < 0)
        p.lineTo(step.x, step.y)
      else
        p.moveTo(step.x, step.y)
    }
  ...
  for (let i = 0; i < rows; i++) {
    const p = createPath()
    const startPoint = {
      x: 3 + random.noise1D(i, 0.1, 2),
      y: map(i, 0, rows, -height * 0.5, height * 1.5),
    }
    const endPoint = {
      x: width - (3 + random.noise1D(i, 0.1, 2)),
      y: startPoint.y - 4
    }
    ...
    for (let j = 0; j <= interp; j++) {
      ...
      step.y += random.noise2D(step.x, step.y, 1, 0.02)
      // p.lineTo(step.x, step.y)
      if (random.noise2D(step.x, step.y, 0.08, 1) < 0.2)
        p.lineTo(step.x, step.y)
      else
        p.moveTo(step.x, step.y)
    }
