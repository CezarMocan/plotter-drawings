...
const drawLine = (p1, p2, resolution, period = 0.05, topLeft, bottomRight, step) => {
  ...
  for (let i = 0; i <= resolution; i++) {
    ...
    if (!currContains) {
      ...
      minYs[i] = currP.y
      const rnd = random.noise2D(currP.y, currP.x / (random.noise2D(currP.x, currP.y, 1, 40) + 41), 0.75, 1)
      if (random.noise1D(currP.x + Math.cos(currP.y * 2) * 5, 0.1, 30) % 4 > 1)
        if (rnd < 0.1 + (2 * currP.y) / 50) p.lineTo(currP.x, currP.y)
        else {
          // p.moveTo(currP.x, currP.y)
          // if (random.noise2D(currP.x, currP.y, 1, 1) < -0.5) 
            // p.lineTo(currP.x - random.noise2D(currP.x, currP.y, 0.08, 3), currP.y - random.noise2D(currP.x, currP.y, 0.17, 4))
            // p.arc(currP.x, currP.y, random.noise2D(currP.x, currP.y, 0.1, 0.2) + 0.5, 0, Math.PI * 2)
          p.moveTo(currP.x, currP.y)
        }
      else
        if (rnd < 0.5) p.moveTo(currP.x, currP.y)
        else p.lineTo(currP.x, currP.y)
    } else {
      if (!prevContains && i > 0) {
...
const sketch = (props) => {
  ...
  for (let i = 0; i < 400; i++) {
    ...
    spacing = (0.07 + (0.01 * Math.cos(i / 15))) * (400 - i) * 0.007
