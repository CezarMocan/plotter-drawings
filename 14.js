...
const generateCircle = (radius, offsetX, offsetY, width, height, noPoints, f1, f2, dots, additionalCircleOffset) => {
  const circleOffset = Math.PI / 12 + additionalCircleOffset
...
const sketch = (props) => {
  ...
  for (let column = 0; column < noColumns; column++) {
    ...
    let topCheckpoints = []
    let y = -24 - column / 2
    let offsetX = random.noise1D(column, 0.6, 13) - 3
    let heightFactor = 1
    ...
    for (let i = 0; i < 39; i++) {
      ...
      let radius = (random.noise1D(i + 100000, 0.5, 0.05) + map(i, 0, 39, 17, 17))
      generateCircle(radius, x, y, width, height, noPoints, f1, f2, dots, circleOffset)
      dotCheckpoints.push(dots)

      circleOffset += 0.02 * Math.sin(i / 5)

      if (i == 0) {
        topCheckpoints.push(dots.slice(0))
        let topDots = [], topSticks = []
        generateCircle(0.1, x, y, width, height, noPoints, f1, f2, topDots, circleOffset)
