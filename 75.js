...
const sketch = (props) => {
  ...
  const R = 25
  const N = 250
  const center = { x: width / 3, y: height * 2.5 / 3 }
  ...
  for (let i = 0; i <= N; i++) {
    ...
    const radius = R + random.noise1D(i, 0.02, 3) * noiseFactor * (1 + Math.sin(i / 10) * 3.3)
    const edge = {
      x: center.x + radius * Math.cos(angle) * (Math.cos(i / 20) + 2) * 2,
      y: center.y + radius * Math.sin(angle) * 2
    }
    const p = createPath()
    const interpFactor = 0//0.45 + random.noise1D(i, 0.018, 0.35) * noiseFactor
    ...
    const noDivisions = 300

    for (let j = 0; j < noDivisions; j++) {
      const currPoint = {
        x: interpolate(edge.x, startPoint.x + Math.sin(j / 10) * 0.15, j / noDivisions),
        y: interpolate(edge.y, startPoint.y + Math.cos(j / 10) * 0.15, j / noDivisions),
      }
      if (j == 0) p.moveTo(currPoint.x, currPoint.y)
      else {
        p.lineTo(
          currPoint.x + random.noise2D(currPoint.x, currPoint.y, 4.1, 0.12),
          currPoint.y + random.noise2D(currPoint.x, currPoint.y, 3.0813, 0.12),
        )
      }
    }
    ...
    for (let j = 0; j < noDivisions; j++) {
      const currPoint = {
        x: interpolate(edge.x, startPoint.x + random.noise2D(startPoint.x, startPoint.y, 0.1, 0.2 * j / noDivisions), j / noDivisions),
        y: interpolate(edge.y, startPoint.y + random.noise2D(startPoint.x, startPoint.y, 0.123, 0.2 * j / noDivisions), j / noDivisions),
      }
      if (j == 0) p.moveTo(currPoint.x, currPoint.y)
      else {
        p.lineTo(
    ...
    for (let j = 0; j < noDivisions; j++) {
      const currPoint = {
        x: interpolate(edge.x, startPoint.x + Math.sin(j / 10.5) * 0.3 * j / noDivisions, j / noDivisions),
        y: interpolate(edge.y, startPoint.y + Math.cos(j / 10.5) * 0.3 * j / noDivisions, j / noDivisions),
      }
      if (j == 0) p.moveTo(currPoint.x, currPoint.y)
      else {
        p.lineTo(
          currPoint.x + random.noise2D(currPoint.x, currPoint.y, 1.1, 0.1 * j / noDivisions),
          currPoint.y + random.noise2D(currPoint.x, currPoint.y, 1.0813, 0.1 * j / noDivisions),
        )
      }
    }
