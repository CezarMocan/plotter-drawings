...
const sketch = (props) => {
  ...
  for (let i = 0; i <= N; i++) {
    ...
    for (let j = 0; j <= M; j++) {
      field[i].push({
        direction: map(random.noise2D(i, j, 0.05, 1), -1, 1, Math.PI, Math.PI + 0.5 * Math.PI)
      })
  ...
  for (let i = 0; i <= width; i += 3.5) {
    for (let j = 0; j <= height; j += 0.15) {
      // const startPoint = { x: i + random.noise2D(i, j, 1, 0.25), y: j + random.noise2D(i, j, 0.03, 0.15) }
      const startPoint = { x: width / 2, y: j + random.noise2D(i, j, 0.03, 0.15) }
      drawLine({
        x: startPoint.x,
        y: startPoint.y,
        step: 0.1,
        direction: null,
        length: 20,// + Math.sin((i * height + j) / 500) * 10,
        path: createPath()
      })    
    }
  }
