...
const drawOutline = ({ x1, y1, r1, x2, y2, r2, direction, radius }, { drawLeft, drawRight, drawCenter }) => {
  ...
  let lengthTotalSplits = parseInt(map(r1, 0.5, 0.05, 15, 2))
  let widthTotalSplits = parseInt(map(r1, 0.5, 0.05, 15, 2))
...
const generatePath = (x, y, radius, direction, weight, depth, maxDepth, inBoundsFn) => {
  ...
  for (let splitSgn of sgns) {
    let isSplitting = (Math.random() < (0.32 * Math.abs(Math.sin(depth / 4))))  
    splitsForSign.push(isSplitting)
  }
  let mainBranchContinues = inBoundsFn(endPoint.x, endPoint.y, radius) && (depth < 40)
  ...
  const newDirection = direction + random.noise2D(x, y, 3, 0.3)
  ...
  if (mainBranchContinues) {
    splits.push({ x: newCenter.x, y: newCenter.y, radius: newRadius * 0.96, direction: newDirection, boundX: endPoint.x, boundY: endPoint.y, sgn: 0 })
  }
