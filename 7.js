...
const sketch = (props) => {
  ...
  let dots = [], dots2 = [], sticks = []

  const noPoints = 1500
  const weightLimit = noPoints
  for (let i = 0; i < noPoints; i++) {
    const pos = getPosition(noPoints, i, width, height, 2, 0, 32850)
  ...
  for (let i = 0; i < noPoints; i++) {
    const pos = getPosition(noPoints, i, width, height, -2, 0, 37850)
    const weightVal = Math.sin(((weightLimit - i) / weightLimit) / 950) / 1000 + random.noise1D(i, 0.008, 0.008)
    dots2.push(new Dot(pos.x, pos.y, createVector(0, weightVal))); 

    if (i > 0) {
      sticks.push(new Stick(dots2[i - 1], dots2[i]))
    }
  }
  ...
  for (let frames = 0; frames < 1; frames++) {
    ...
    const pP = createPath()
    for (let i = 0; i < noPoints; i++) {
      pP.moveTo(dots[i].pos.x, dots[i].pos.y)
      pP.lineTo(dots2[i].pos.x, dots2[i].pos.y)
    }
    paths.push(pP)
